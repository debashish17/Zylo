...
Write-Host "`n📦 Step 1: Installing Node.js dependencies..." -ForegroundColor Cyan
pnpm install
...
Write-Host "`n🐍 Step 2: Setting up Python virtual environments..." -ForegroundColor Cyan
...
Write-Host "`n🔐 Step 3: Setting up environment variables..." -ForegroundColor Cyan

if (-not (Test-Path "apps/web/.env.local")) {
    Write-Host "    Creating apps/web/.env.local from example..." -ForegroundColor Gray
    Copy-Item ".env.example" "apps/web/.env.local"
    Write-Host "    ⚠️  Please edit apps/web/.env.local with your Supabase credentials" -ForegroundColor Yellow
}

# Step 4: Create uploads directory
Write-Host "`n📁 Step 4: Creating uploads directory..." -ForegroundColor Cyan
...
Write-Host "`n🎮 Step 5: Testing GPU..." -ForegroundColor Cyan
